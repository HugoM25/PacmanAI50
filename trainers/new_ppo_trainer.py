...
class NewPPOTrainer(Trainer):
    ...
    def train(self, max_iterations: int) -> None:
        ...
        while step < max_iterations :
            ...
            # Play episodes until we have enough experience
            while batch_steps < self.timesteps_per_batch:
                ...
                while ep_step < self.max_timesteps_per_episode and not done:
                ...
                print(f"Episode done. Timesteps run: {ep_step + 1}, Episode reward: {np.sum(ep_rews[0])}")


                for i in range(self.env.nb_agents):
                    # Collect episodic length and rewards
                    batch_lens[i].append(ep_step + 1)
            ...
            # Update the model
            self.update_model(batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens)

            # Save the model every 10000 iterations
            if step % 1000 == 0:
                self.save_model(f"actor_{step}.pth", f"critic_{step}.pth")
    ...
    def update_model(self, batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens) -> None:
